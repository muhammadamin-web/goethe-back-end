...
@section('header', 'A2 Test Dates')

@section('content')
    <div class="container">
        <h2>A2 Test Dates Management</h2>
        <a href="{{ route('limits_a2.create') }}" class="btn btn-success mb-3">Create New A2 Test</a>
            ...
            <tbody>
                @foreach($limits as $limit)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('limits_a2.edit', $limit->id) }}" class="btn btn-warning btn-sm">Edit</a>
                            <form action="{{ route('limits_a2.destroy', $limit->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
