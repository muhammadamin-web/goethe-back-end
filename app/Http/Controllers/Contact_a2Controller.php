...
class Contact_a2Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Contact_a2::query();
        if ($testId) {
            $query->where('test_id', $testId);
        }
        $contacts = $query->get();
        $tests = Limit_a2::all();
        return view('contacts.index_a2', compact('contacts', 'tests', 'testId'));
    }

    public function countContacts()
    {
        return response()->json(['count' => Contact_a2::count()]);
    }
    ...
    public function submitForm(Request $request)
    {
        try {
            ...
            $limit = Limit_a2::find($request->test_id);
            ...
            $currentCount = Contact_a2::where('test_id', $request->test_id)->count();
            ...
            $validatedData = $request->validate([
                ...
                'test_id' => 'required|integer|exists:limit_a2s,id'
            ]);

            // Telefon raqami tekshirish
            if (Contact_a2::where('phone_number', $validatedData['phone_number'])->exists()) {
